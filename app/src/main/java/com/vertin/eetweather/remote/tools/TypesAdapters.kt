...
class TypesAdapterFactory : JsonAdapter.Factory {
    private val types = mapOf<Type, JsonAdapter<*>>(
        ...
        String::class.java to EmptyStringAdapter()
    )
...
internal class EmptyStringAdapter : XNullableJsonAdapter<String?>() {
    ...
    override fun fromNonNullString(nextString: String): String? {
        return if (nextString.isEmpty()) {
            null
        } else {
            nextString
        }
